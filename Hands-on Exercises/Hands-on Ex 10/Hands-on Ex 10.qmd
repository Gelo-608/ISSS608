...
```{r}
pacman::p_load(lubridate, ggthemes, reactable,reactablefmtr, gt, gtExtras, tidyverse,dplyr)
```
...
```{r}
coffeechain <- readRDS("/Users/geloliu/Gelo-608/ISSS608/Hands-on Exercises/Hands-on Ex 10/data/CoffeeChain.rds")
```
...
⚠️ Remember install gtExtras and svglite packages first.

```{r}
pacman::p_load(gtExtras,svglite)
```
