...
```{r}
sgpools <- read_csv("/Users/geloliu/Gelo-608/ISSS608/Hands-on Exercises/Hands-on Ex 08/data/aspatial/SGPools_svy21.csv")
```
...
```{r}
tm_shape(sgpools_sf) +
  tm_bubbles(col = "red",  # ✅ 仍然使用 `col`
             size = 1, 
             border.col = "black",  # ✅ 使用 `border.col`
             border.lwd = 1)  # ✅ 使用 `border.lwd`
